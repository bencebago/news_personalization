...
tab_model(model4_vote, model4_bookmark,
          dv.labels = c("Vote", "Bookmark"),
          pred.labels = pred_labels,
          title = "Table S4. Regression Results for Prior on Climate Change is a Threat",
          show.ci = FALSE, 
          show.p = TRUE,
          show.se = TRUE, 
          show.re.var = FALSE,  
...
When restricting analysis to skeptics (as per the categorical variable) we find a significant treatment effect on upvotes ($b = -0.1, p = 0.014$), but not on bookmarks ($b = -0.06, p = 0.157$). This analysis is exploratory since the preregistered outcome of interest was the interaction effect between belief and treatment. A post-hoc power analysis suggested that we would have needed about 1,000 skeptics to detect the $b = -0.06$ effect size on bookmarks with a 95\% power. We conducted a siulation based power analysis, for which we used the parameters (standard errors, beta) from the corresponding model. 

```{r power analysis, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE, eval=FALSE}



# SDs are taken from model1_bookmark
prolific_sd <- 0.078
title_sd <- 0.154
resid_sd <- 0.468
...
run_sim <- function() {
  subj_ids <- 1:n_subj
  item_ids <- 1:n_items_total
  
  # Assign treatment randomly across subjects
  treatment_df <- tibble(
    subject = subj_ids,
    treatment = rbinom(n_subj, 1, 0.5)
  )
  
  # Randomly assign 11 items to each subject
  subject_item_df <- treatment_df %>%
    rowwise() %>%
    mutate(items = list(sample(item_ids, n_items_per_subj))) %>%
    unnest(items) %>%
    rename(item = items)
  
  # Simulate random intercepts
  subj_re <- rnorm(n_subj, 0, prolific_sd)
  item_re <- rnorm(n_items_total, 0, title_sd)
  
  df <- subject_item_df %>%
    mutate(
      subj_intercept = subj_re[subject],
      item_intercept = item_re[item],
      error = rnorm(n(), 0, resid_sd),
      bookmark_prob = 0.43 + b_treat * treatment + subj_intercept + item_intercept + error,
      bookmark_prob = pmin(pmax(bookmark_prob, 0), 1),
      bookmark = rbinom(n(), 1, bookmark_prob)
    )
  
  # Fit model
  model <- lmer(bookmark ~ treatment + (1 | subject) + (1 | item), data = df)
  pval <- summary(model)$coefficients["treatment", "Pr(>|t|)"]
  beta <- fixef(model)["treatment"]
  
  return(tibble(pval = pval, beta = beta))
}

# Run simulation 1000 times to estimate power
set.seed(42)
n_sim <- 1000
results <- map_dfr(1:n_sim, ~run_sim())

power_estimate <- mean(results$pval < 0.05)
mean_beta <- mean(results$beta)

#cat("Estimated power:", round(power_estimate, 3), "\n")
#cat("Mean estimated beta:", round(mean_beta, 3), "\n")
...
Tables S9-S10 summarize the results. We conducted this analysis using the dichotomous prior and the continuous prior on the belief that climate change is caused by human activity, as we found evidence for decreased regret among skeptics in the treatment condition only in these 2 measures. Indeed, we found evidence that one sentimentality score, as rated by Llama3, included in the regression, treatment condition no longer significantly interact with prior beliefs, while sentimentality does interact with prior beliefs significantly. This supports our suspicion that negativity of the article texts makes skeptics experience negative emotions that make them later regret their choice. Nevertheless, the interpretation of this analysis is speculative and should be treated as such. Importantly, as it can be seen in the analysis of the posterior beliefs or the perceived credibility of the article this increased regret in the modified headline condition does not translate to decreased belief updating nor a decrease in judgments of credibility.
